      ...
      sc.forEach(obj=>{
          Course.findById(obj.courseID,(err,course)=>{
              ...
              if(course.name==courseName){
              	  Submission.find({courseName:courseName,studentID:user._id},(err,subs)=>{
              	  	//console.log(subs.assName);
              	  }) 
              	  var t=0;
              	  var names = [];
              	  var grades = [];
                  var total =0 ;
                  Submission.find({courseName:courseName,studentID:user._id},(err,subs)=>{
                  	//console.log(subs.grade);
                  	
                  	subs.forEach(sub=>{
                  		t=t+1;
                      		var grade =(sub.grade);
                      		var assName = sub.assName;
                      		names = names + assName;
                      		grades = grades + grade;
                      		console.log(grades);
                      		Assignment.findOne({nameofA:assName},(err,ass)=>{
                      			var weightage = ass.weightage;
                      			total= total + weightage * grade ;
                      			//res.send(" Total Grade ( 0-10 ) : " + total/100);
                      			
                      		if(t==subs.length){
                      		//console.log(names);
                      			 res.render("grades_student",{
            					courseName:courseName,
            					total : total/100,
            					subs:subs,
            					names : names,
